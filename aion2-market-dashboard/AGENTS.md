...
- Linked-account filtering uses `linkedAccountLabel` values `4连号` through `8连号`. Prefer explicit title/detail text like `连体号-5连号`; otherwise infer `main account + parsed small accounts`, so descriptions like `小号158杀+181护+154弓` count as `4连号`.
