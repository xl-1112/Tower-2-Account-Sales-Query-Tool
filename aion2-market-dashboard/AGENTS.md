...
- Linked-account filtering uses `linkedAccountLabel` values `4连号` through `8连号`. For `螃蟹`, prefer API tags such as `important: ["同职业4连号"]`; otherwise prefer explicit title/detail text like `连体号-5连号`; finally infer `main account + parsed small accounts`, so descriptions like `小号158杀+181护+154弓` count as `4连号`.
