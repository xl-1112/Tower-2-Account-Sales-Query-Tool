...
**Active Feature:** data-015 - 永恒之塔2最高角色等级本地筛选（已完成）
...
- [x] 查询条件新增 `战斗力 ≥` 数字输入；查询按钮按当前已抓取数据本地过滤，不触发平台请求。
- [x] 从卖家说/标题中的明确 `最高角色等级:50` 字段解析 `maxCharacterLevel`；查询条件新增 `等级` 数字输入并在前端按等级精确匹配。
- [x] 修复等级后紧跟连体号时的解析粘连：`最高角色等级:49 4连号` 不再被误读为 494，50 级也不再被误读为 503～508；真实抓取 200 条复核三位异常等级为 0。
...
- 根目录当前缺少 `app.json`、`page-meta.json`、`project.config.json`，`node scripts/validate-wechat-ai-skill.mjs` 因此有 4 个既有基线错误；本次未扩展范围重建微信小程序工程文件。
- 当前 Windows 终端未安装或未暴露 `bash`，因此 `bash init.sh` 无法启动；已分别执行本任务可用的 Node/npm 校验命令。

## Decisions Made

- 优先查询、告警和低风险控制；高风险动作采用申请/审批模式。
- 自动检查声明、注册、路径和组件关联页的一致性，手工检查模型意图与卡片体验。
- 角色交易网站按用户筛选抓取螃蟹和 7881；单平台单次抓取上限由用户分别填写，默认螃蟹 100 条、7881 100 条；默认种族为“全部”，包含天族和魔族；网站分页可在 10、50、100 条之间切换，本地查询、战斗力与等级筛选、排序和翻页均在前端完成，不重复抓取，不保存历史数据。
...
- 战斗力筛选验证：`npm test` 15 项通过；`npm run build`、`node scripts/validate-aion2-data-probe.mjs` 通过
