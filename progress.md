...
- [x] 移除纯 HTTP 抓取迁移后遗留的全局串行队列，避免一轮挂起请求长期阻塞后续重新抓取。
- [x] 为螃蟹和 7881 的单次来源请求增加 15 秒超时；任一来源失败时仍沿用既有部分成功告警机制。
- [x] 聚合结果按 `source + productId` 去重，避免重复行标识导致 React 复用错误的旧首屏节点。
- [x] 前端重新抓取增加 60 秒超时、旧请求取消和最新请求序号校验，过期响应不会覆盖新结果。
- [x] 应用内浏览器真实验证：重新抓取时间由 09:42:21 更新到 09:43:26；首屏 10 条链接全部唯一，加载期间及完成后展开按钮可用，详情链接可用，浏览器无错误。
